...
//! FUSE_RENAME renames an entry from one directory to another.
//!
...
pub struct RenameReq {
    /// Target parent directory node ID.
    newdir: FuseNodeId,
}
...
impl FuseOperation for RenameOperation<'_> {
    ...
    fn body_len(&self) -> usize {
        size_of::<RenameReq>()
            .saturating_add(self.old_name.len())
            .saturating_add(util::NAME_TERMINATOR.len())
            .saturating_add(self.new_name.len())
            .saturating_add(util::NAME_TERMINATOR.len())
    }
