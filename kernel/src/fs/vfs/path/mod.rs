...
/// Returns `true` if `dentry` references the current or an older mount namespace.
///
/// Bind-mounting or moving such a mount-namespace file into the current mount
/// namespace would create a namespace loop.
fn is_mnt_ns_loop(dentry: &Dentry, current_mnt_ns: &Arc<MountNamespace>) -> bool {
    ...
    current_mnt_ns.as_ref() >= mnt_ns_inode.ns().as_ref()
}

/// Ensures that a mount subtree contains no mount-namespace loop.
fn check_no_mnt_ns_loop_in_tree(
    root_mount: &Arc<Mount>,
    current_mnt_ns: &Arc<MountNamespace>,
) -> Result<()> {
    ...
    while let Some(mount) = worklist.pop_front() {
        ...
        if checked_root_dentries.insert(root_dentry.key())
            && is_mnt_ns_loop(root_dentry, current_mnt_ns)
        {
            return_errno_with_message!(
                Errno::ELOOP,
...
impl Path {
    ...
    pub fn bind_mount_to(&self, dst_path: &Self, recursive: bool, ctx: &Context) -> Result<()> {
        ...
        if is_mnt_ns_loop(&self.dentry, current_mnt_ns) {
            return_errno_with_message!(
                Errno::EINVAL,
    ...
    pub fn move_mount_to(&self, dst_path: &Self, ctx: &Context) -> Result<()> {
        ...
        check_no_mnt_ns_loop_in_tree(self.mount_node(), current_mnt_ns)?;
        if dst_path
            .mount_node()
