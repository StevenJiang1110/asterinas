...
use crate::{
    ...
    net::{
        route::{RouteType, is_broadcast_endpoint},
        socket::{
            Socket,
            ip::{
                common::get_ephemeral_endpoint_and_route_type,
        ...
        let (sent_bytes, iface_to_poll) = select_remote_and_bind(
            ...
            || {
                ...
                let (endpoint, route_type) =
                    get_ephemeral_endpoint_and_route_type(remote_endpoint)?;
                if !can_broadcast && route_type == Some(RouteType::BROADCAST) {
