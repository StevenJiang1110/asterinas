...
use crate::{
    ...
    net::socket::{
        ...
        netlink::{is_valid_family_id, NetlinkRouteSocket, StandardNetlinkFamily},
...
pub fn sys_socket(domain: i32, type_: i32, protocol: i32, ctx: &Context) -> Result<SyscallReturn> {
    ...
    let file_like = match (domain, sock_type) {
        ...
        (CSocketAddrFamily::AF_NETLINK, SockType::SOCK_RAW | SockType::SOCK_DGRAM) => {
            match StandardNetlinkFamily::try_from(protocol as u32) {
                Ok(StandardNetlinkFamily::NETLINK_ROUTE) => {
                ...
                Err(_) => {
                    if is_valid_family_id(protocol as u32) {
                        return_errno_with_message!(
                            Errno::EAFNOSUPPORT,
