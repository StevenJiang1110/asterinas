{ config, lib, pkgs, ... }:
let
  splash = builtins.path { path = builtins.getEnv "NIXOS_SPLASH_PATH"; };
...
in {
  ...
  boot.loader.grub.splashImage = splash;
  ...
  boot.postBootCommands = ''
    ...
    if [ "${builtins.getEnv "NIXOS_DISABLE_SYSTEMD"}" = "true" ]; then
      STAGE_2_ARGS="${builtins.getEnv "NIXOS_STAGE_2_ARGS"}"
      "${builtins.getEnv "NIXOS_STAGE_2_INIT"}" $STAGE_2_ARGS
    fi
  '';
