...
show_help() {
  cat <<'EOF'
Usage: bash tools/kata/run_kata_workload.sh

Runs the configurable Kata workload against the already-started
...
Environment:
  ...
  KATA_SNAPSHOTTER              Snapshotter passed to `nerdctl`.
EOF
}
...
if should_pull_test_image; then
  nerdctl --snapshotter "${snapshotter}" --address "${containerd_address}" pull --quiet "${image}" \
    2>&1 | tee "${pull_log_file}"
fi

printf 'image=%s\n' "${image}" > "${run_log_file}"
printf 'runtime=%s\n' "${runtime}" >> "${run_log_file}"
printf 'snapshotter=%s\n' "${snapshotter}" >> "${run_log_file}"
