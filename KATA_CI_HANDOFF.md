...
- Latest repo commit in this handoff state:
  - `83d574b5b` `Fix Kata CI tmpfs mount inspection`
...
- Current refactor status:
  ...
  - latest passing run that validates the Linux guest kernel, default
    `nerdctl` snapshotter, and two-pass workflow logic: `24548532131`
  - `https://github.com/StevenJiang1110/asterinas/actions/runs/24548532131`
...
- forces the Linux guest kernel from the multi-kernel Asterinas Kata release
  with `KATA_GUEST_KERNEL=linux`
- installs `containerd`, `nerdctl`, `crictl`, Kata artifacts, and related packages
- runs `tools/kata/check_overlayfs.sh` before install to verify that host-side
  overlayfs upper/work staging is usable
...
- Overlayfs preflight: `tools/kata/check_overlayfs.sh`
...
- The validated outer-container requirements are:
  - `--privileged`
  - `--cgroupns host`
  - a non-overlay-backed staging area for host-side overlayfs upper/work
    directories; CI currently provides this with `tmpfs` mounts on `/tmp` and
    `/var/lib/containerd`
...
- The inner `containerd` / `nerdctl` stack no longer forces snapshotter
  `native`.
- Default `nerdctl` snapshotter behavior works once `/var/lib/containerd` is on
  `tmpfs`; without that, containerd falls back to host-side `overlayfs` on top
  of the job container's overlay rootfs and Kata fails with
  `failed to create shim task: invalid argument`.
- The guest shared filesystem remains `virtio-fs`; the host snapshotter choice
  is only for host-side rootfs staging.

Validation runs:

- latest passing run with Linux guest kernel and `tmpfs` overlay staging:
  `https://github.com/StevenJiang1110/asterinas/actions/runs/24548532131`
- failure that reproduced default snapshotter on outer overlayfs backing:
  `https://github.com/StevenJiang1110/asterinas/actions/runs/24546258768`
- passing run with Linux guest kernel before removing forced `native`:
  `https://github.com/StevenJiang1110/asterinas/actions/runs/24545819068`
...
1. Did the workflow lose either required job-container option?
   - `--privileged`
   - `--cgroupns host`
2. Did the workflow lose either CI `tmpfs` staging mount?
   - `/tmp`
   - `/var/lib/containerd`
3. Does `tools/kata/check_overlayfs.sh` still pass in CI?
4. Is the workflow still forcing `KATA_GUEST_KERNEL=linux`?
5. Do the job logs still show the key failure output:
   - `kata-check.txt`
   - `containerd.log`
...
- the GitHub Actions workflow uses the Linux guest kernel from the Asterinas
  Kata release because the packaged Asterinas guest kernel path still cannot
  complete the Kata agent handshake
- CI no longer forces `nerdctl --snapshotter native`
- CI mounts `/tmp` and `/var/lib/containerd` as `tmpfs` so containerd's default
  overlayfs snapshotter can put upper/work directories on a non-overlay-backed
  filesystem
- `tools/kata/check_overlayfs.sh` documents and verifies this environment
  requirement before the Kata install step
