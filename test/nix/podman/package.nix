...
let
  ...
  binPath = lib.makeBinPath (lib.optionals stdenv.hostPlatform.isLinux [
    ...
    iptables
    iproute2
  ] ++ lib.optionals stdenv.hostPlatform.isDarwin [ vfkit ] ++ extraPackages);
  ...
  helpersBin = symlinkJoin {
    ...
    paths = [ gvproxy ] ++ lib.optionals stdenv.hostPlatform.isLinux [
      aardvark-dns
...
in buildGoModule rec {
  ...
  patches = [
    (replaceVars ./hardcode-paths.patch { bin_path = helpersBin; })
