...
#define UNIX_STREAM_CONNECT()                                                 \
	do {                                                                  \
		int fds[2] = { -1, -1 };                                      \
		TEST_SUCC(socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, \
				     fds));                                   \
		send_fd = fds[0];                                             \
		recv_fd = fds[1];                                             \
	} while (0)
...
#define SOCKETPAIR_CONNECT(socket_type) \
	TEST_SUCC(socketpair(AF_UNIX, (socket_type) | SOCK_NONBLOCK, 0, fds))
...
#define WAIT_READABLE() TCP_WAIT_READABLE()
...
FN_TEST(unix_seqpacket_msg_peek_with_scm_rights)
{
	...
	TEST_SUCC(socketpair(AF_UNIX, SOCK_SEQPACKET | SOCK_NONBLOCK, 0, fds));
...
FN_TEST(netlink_raw_msg_peek)
{
	...
	TEST_SUCC(send_getlink_request(fd, 1));
...
FN_TEST(netlink_dgram_msg_peek)
{
	...
	fd = TEST_SUCC(
		socket(AF_NETLINK, SOCK_DGRAM | SOCK_NONBLOCK, NETLINK_ROUTE));
	TEST_SUCC(bind(fd, (struct sockaddr *)&addr, sizeof(addr)));

	TEST_SUCC(send_getlink_request(fd, 2));
